...
(function productionUI(){
  ...
  async function render(tab){
    ...
    if(tab==="Settings"){
      ...
      body.querySelector("#sSave").onclick=async()=>{localStorage.setItem("dc_ai_timeout",body.querySelector("#sTimeout").value);const d=await post("/api/settings",{ai_timeout:body.querySelector("#sTimeout").value,project_build_command:body.querySelector("#sBuild").value,project_test_command:body.querySelector("#sTest").value}).catch(e=>({error:String(e)}));body.querySelector("#sLog").textContent=JSON.stringify(d,null,2);toast(d.error?"Settings save failed":"Settings saved","success")}; return;
    }
    if(tab==="Recovery"){
