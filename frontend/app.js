...
(function productionUI(){
  ...
  async function render(tab){
    ...
    if(tab==="GitHub"){
      ...
      const installs=me.connected?await api("/api/github/installations").catch(()=>({installations:[]})):{installations:[]};
      const firstInstall=installs.installations?.[0]?.id||"";
      const repos=me.connected?await api("/api/github/repositories?installation_id="+encodeURIComponent(firstInstall)).catch(()=>api("/api/github/repositories").catch(()=>({repositories:[]}))):{repositories:[]};
      body.innerHTML='<h3>GitHub connection</h3><div class="muted">'+(me.connected?"Connected as "+escapeHtml(me.login):"Not connected")+'</div><div class=dc-row>'+(me.connected?'<button id="dcDisconnect">Disconnect</button>':'<button id="dcConnect">Connect GitHub</button>')+'</div><div class=dc-grid><select id=dcInstall><option value="">All accessible repositories</option>'+((installs.installations||[]).map(x=>'<option value="'+x.id+'">'+escapeHtml(x.account||("Installation "+x.id))+'</option>').join(""))+'</select><select id=dcRepo>'+repos.repositories.map(x=>'<option value="'+escapeHtml(x.full_name)+'">'+escapeHtml(x.full_name)+'</option>').join("")+'</select><input id=dcBranch placeholder="branch (main)"/></div><div class=dc-row><button id=dcRepoSet>Use repository</button><button id=dcRefreshRepos>Refresh repositories</button></div><div id=dcGhLog class=dc-log></div>';
      body.querySelector("#dcConnect")?.addEventListener("click",()=>{ if(!cfg.configured){toast("Set DREAMCODER_GITHUB_CLIENT_ID/SECRET and OAUTH_STATE_SECRET first","error");return;} window.open(API_BASE+"/api/github/oauth/start","_blank","width=900,height=800"); let n=0; const poll=setInterval(async()=>{n++; const s=await api("/api/github/oauth/status").catch(()=>({connected:false})); if(s.connected||n>20){clearInterval(poll);render("GitHub");}},1500);});
      ...
      body.querySelector("#dcInstall")?.addEventListener("change",async()=>{const id=body.querySelector("#dcInstall").value;const rr=await api("/api/github/repositories"+(id?"?installation_id="+encodeURIComponent(id):"")).catch(()=>({repositories:[]}));const sel=body.querySelector("#dcRepo");sel.innerHTML=(rr.repositories||[]).map(x=>'<option value="'+escapeHtml(x.full_name)+'">'+escapeHtml(x.full_name)+'</option>').join("");}); body.querySelector("#dcRefreshRepos")?.addEventListener("click",()=>render("GitHub"));
      return;
    }
    if(tab==="Git"){
      ...
      const files=await api("/api/git/workflow/files").catch(()=>({files:[]})); body.innerHTML='<h3>Git workflow</h3><div class=dc-row><button data-git=fetch>Fetch</button><button data-git=pull>Pull</button><button data-git=push>Push</button><button data-git=stash>Stash</button><button data-git=merge>Merge</button><button id=dcConflict>Resolve conflict</button><button id=dcAgentBranch>Agent branch</button><button id=dcAgentPr>Push + PR</button></div><div class=dc-grid><input id=dcBranchName placeholder="new/switch branch"/><button data-git=branch>Create & switch branch</button><input id=dcCommit placeholder="commit message"/><button data-git=commit>Commit staged</button></div><div class=dc-log id=dcGitFiles>'+((files.files||[]).map(f=>'<div><button data-stage="'+escapeHtml(f.path)+'">'+(f.index!==" "?"Unstage":"Stage")+'</button> <code>'+escapeHtml(f.path)+'</code></div>').join("")||"Working tree clean")+'</div><pre class=dc-log>'+escapeHtml(JSON.stringify({status:s,branches:br},null,2))+'</pre>';
      body.querySelectorAll("[data-stage]").forEach(x=>x.onclick=async()=>{const path=x.dataset.stage;const f=(files.files||[]).find(z=>z.path===path);const d=await post("/api/git/workflow/"+(f&&f.index!==" "?"unstage":"stage"),{paths:[path]});toast(d.ok?"Git index updated":"Git operation failed",d.ok?"success":"error");render("Git");});
      body.querySelector("#dcConflict")?.addEventListener("click",async()=>{const path=prompt("Conflicted file path");if(!path)return;const d=await api("/api/git/workflow/conflict-data?path="+encodeURIComponent(path));const merged=await post("/api/files/merge",{base:d.base,current:d.current,incoming:d.incoming});showModal({title:"Three-way merge: "+path,bodyHtml:'<div class="dc-grid"><textarea id="mCurrent" style="min-height:180px">'+escapeHtml(d.current)+'</textarea><textarea id="mIncoming" style="min-height:180px">'+escapeHtml(d.incoming)+'</textarea></div><pre class="dc-log">'+escapeHtml(merged.content||"")+'</pre>',applyLabel:"Apply merge",onApply:async()=>{const x=await post("/api/editor/recovery",{path,content:merged.content||""});toast(x.ok?"Merged file written; stage it to finish":"Merge write failed",x.ok?"success":"error")}});});
      body.querySelector("#dcAgentBranch")?.addEventListener("click",async()=>{const name="agent/"+new Date().toISOString().replace(/[:.]/g,"-");const d=await post("/api/git/workflow/agent-branch",{name});toast(d.ok?"Agent branch created":"Branch failed",d.ok?"success":"error");render("Git");});
      body.querySelector("#dcAgentPr")?.addEventListener("click",async()=>{const branch=prompt("Agent branch name");if(!branch)return;const d=await post("/api/git/workflow/agent-pr",{branch,title:prompt("PR title")||"DreamCoder Agent changes",body:"Generated and verified by DreamCoder"});toast(d.ok?"Pull request created":"PR failed",d.ok?"success":"error");});
      body.querySelectorAll("[data-git]").forEach(x=>x.onclick=async()=>{const op=x.dataset.git;let d;if(op==="branch")d=await post("/api/git/workflow/branch",{name:body.querySelector("#dcBranchName").value});else if(op==="commit")d=await post("/api/git/workflow/commit",{message:body.querySelector("#dcCommit").value});else if(op==="merge")d=await post("/api/git/workflow/merge",{branch:prompt("Branch to merge")||""});else d=await post("/api/git/workflow/"+op,{});toast(d.ok?"Git operation complete":"Git operation failed",d.ok?"success":"error");render("Git");});
    ...
    if(tab==="Settings"){
      ...
      body.querySelector("#sSave").onclick=async()=>{localStorage.setItem("dc_ai_timeout",body.querySelector("#sTimeout").value);const d=await post("/api/settings",{ai_timeout:body.querySelector("#sTimeout").value,project_build_command:body.querySelector("#sBuild").value,project_test_command:body.querySelector("#sTest").value,agent_unrestricted:body.querySelector("#sMode").value==="unrestricted",agent_network:body.querySelector("#sNet").value==="1"}).catch(e=>({error:String(e)}));body.querySelector("#sLog").textContent=JSON.stringify(d,null,2);toast(d.error?"Settings save failed":"Settings saved","success")}; return;
    }
    if(tab==="Recovery"){
