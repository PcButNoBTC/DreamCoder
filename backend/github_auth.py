...
async def exchange(code):
    ...
    if not available():
        raise RuntimeError("OS keychain is required for GitHub OAuth token storage")
    set_secret("github_oauth_token", d["access_token"])
    if d.get("refresh_token"):
        set_secret("github_oauth_refresh_token", d["refresh_token"])
    db.set_setting("github_oauth_expires_at", str(time.time() + int(d.get("expires_in", 28800))))
    db.set_setting("github_oauth_refresh_expires_at", str(time.time() + int(d.get("refresh_token_expires_in", 15897600))))
