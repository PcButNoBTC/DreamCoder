...
def audit(event:str,**data:Any)->None:
    from pathlib import Path
    p=Path(os.getenv("DREAMCODER_AUDIT_LOG","~/.dreamcoder/audit.jsonl")).expanduser(); p.parent.mkdir(parents=True,exist_ok=True)
    import json,time
    safe={k:redact(str(v)) for k,v in data.items()}
    with p.open("a",encoding="utf-8") as f: f.write(json.dumps({"ts":time.time(),"event":event,**safe})+"\n")
